							...
							<tbody>
								 <?php $cnt=0; if(count($page_data)>0){
									foreach($page_data as $row){
										$cnt++;
										$catName="";
										if($row['child_cat_id']!=0)
										{
										$chCatIds=explode(',',$row['child_cat_id']);
										
										for($i=0;$i<count($chCatIds); $i++)
										{
										$sql=$this->db->query("select * from category where cat_id=".$chCatIds[$i]."");
										$res=$sql->row_array();
										extract($res);
										if($catName=="")
										{
								...
								<tr id="" class="odd gradeX"> 
									...
									<td class="hidden-phone"><?php if($catName!=""){ echo $catName; }else{ echo "Not Created";} ?></td>
